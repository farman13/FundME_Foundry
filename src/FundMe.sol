...
contract FundMe {
    ...
    function withdrawCheaper() public onlyOwner {
        uint FundersLength = s_funders.length; // Instead of calling s_funders.length multiple times from storage(which results in more gas consumption)  we just call it one time & stores it temprory.
        for (
            uint256 funderIndex = 0;
